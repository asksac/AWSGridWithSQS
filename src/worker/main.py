  ...
  while True: 
    ...
    if iter_time >= 1.0 or reset_tps: 
      if tps: 
        ...
        statsdpipe.guage(STATS_PREFIX + '.worker.tps', tps)
        statsdpipe.guage(STATS_PREFIX + '.worker.iter_time', iter_time)
        statsdpipe.guage(STATS_PREFIX + '.worker.cet', cet)
        statsdpipe.guage(STATS_PREFIX + '.worker.crmt', crmt)
        statsdpipe.guage(STATS_PREFIX + '.worker.csmt', csmt)
        statsdpipe.guage(STATS_PREFIX + '.worker.cdmt', cdmt)
      else: 
        statsdpipe.guage(STATS_PREFIX + '.worker.tps', 0)
        statsdpipe.guage(STATS_PREFIX + '.worker.iter_time', iter_time)
        statsdpipe.guage(STATS_PREFIX + '.worker.cet', cet)
        statsdpipe.guage(STATS_PREFIX + '.worker.crmt', crmt)
        statsdpipe.guage(STATS_PREFIX + '.worker.csmt', csmt)
